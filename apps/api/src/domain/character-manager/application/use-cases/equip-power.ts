...
interface EquipPowerUseCaseRequest {
  characterId: string;
  userId: string;
  powerId: string;
}

type EquipPowerUseCaseResponse = Either<
...
@Injectable()
export class EquipPowerUseCase {
  ...
  async execute({
    ...
    userId,
    powerId,
  }: EquipPowerUseCaseRequest): Promise<EquipPowerUseCaseResponse> {
