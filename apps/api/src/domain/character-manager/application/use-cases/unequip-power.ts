...
interface UnequipPowerUseCaseRequest {
  characterId: string;
  userId: string;
  powerId: string;
}

type UnequipPowerUseCaseResponse = Either<
...
@Injectable()
export class UnequipPowerUseCase {
  ...
  async execute({
    ...
    userId,
    powerId,
  }: UnequipPowerUseCaseRequest): Promise<UnequipPowerUseCaseResponse> {
